...
3. Classify:
   - existing + directly definitional or semantically constitutive for this page's first line → link the canonical page from this page
   - existing + explanatory, derivative, incidental, or detail of a first-line concept → keep it as prose on this page; link it on the page that owns that detail instead. An existing page does not make every mention edge-worthy
   - missing + potentially directly definitional → ask the human operator whether to promote it before creating a page
   - missing + explanatory, derivative, illustrative, or not independently graph-worthy → keep it as prose or a facet; record deferral only when later reconsideration is useful
4. Record the promotion status of every Title-Case or otherwise concept-like term that does substantive explanatory work on each curated page. For every such term, distinguish the link decision from the promotion decision: a direct definitional dependency gets a first-line link; an existing explanatory or derivative concept is kept as prose and owned by its appropriate page; an illustrative or non-node term is kept as prose and labeled accordingly; an unresolved promotion question is reported to the operator. Do not leave a concept-like term silently unclassified in prose. On ordinary concept pages, use explicit sections such as `## Direct definitional relations`, `## Vault concepts kept as prose`, and `## Illustrations intentionally kept as prose` when applicable; on organizing pages, use the three-set structure below.
